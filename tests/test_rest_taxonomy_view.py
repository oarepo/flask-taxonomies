...
def get_excluded_title_descendants_test(api, client, excluded_title_sample_taxonomy):
    taxonomy = client.get('/api/2.0/taxonomies/test?representation:include=dsc')
    if taxonomy.status_code != 200:
        print(taxonomy.data)
    assert taxonomy.status_code == 200
...
def get_taxonomy_term_parent_test(api, client, sample_taxonomy):
    taxonomy = client.get('/api/2.0/taxonomies/test/a/aa?representation:include=par&representation:exclude=anc')
    if taxonomy.status_code != 200:
        print(taxonomy.data)
    assert taxonomy.status_code == 200
    assert json.loads(taxonomy.data) == {
...
def get_taxonomy_term_ancestor_tag_test(api, client, sample_taxonomy):
    taxonomy = client.get('/api/2.0/taxonomies/test/a/aa?representation:include=ant anl')
    if taxonomy.status_code != 200:
        print(taxonomy.data)
    assert taxonomy.status_code == 200
    assert json.loads(taxonomy.data) == [
...
def get_taxonomy_term_ancestor_tag_parent_test(api, client, sample_taxonomy):
    taxonomy = client.get('/api/2.0/taxonomies/test/a/aa?representation:include=ant anl par')
    if taxonomy.status_code != 200:
        print(taxonomy.data)
    assert taxonomy.status_code == 200
    assert json.loads(taxonomy.data) == [
